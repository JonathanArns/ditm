...
func (s *server) getCurrentPeers() map[int]string {
	...
	for i, e := range s.logs {
		if i > s.committedIndex {
			break // this fixes the redis-raft split-brain
		}
		if e.ServerAddr != "" {
...
func (s *server) handle(w http.ResponseWriter, r *http.Request) {
	...
	case "request_vote":
		...
		// BUG: removing the 2nd part of the condition is required as well to enable the elasticsearch bug in raft
		// if req.Term < s.currentTerm || s.cummulativeHeartbeatTimeout < minElectionTimeoutMultiplier*electionTimeout {
		if req.Term < s.currentTerm {
			data, _ := json.Marshal(response{Term: s.currentTerm})
			w.Write(data)
			log.Println("DEBUG1:", s.currentTerm, req.Term, s.cummulativeHeartbeatTimeout, minElectionTimeoutMultiplier*electionTimeout)
			return
		} else if req.Term > s.currentTerm {
			s.role = FOLLOWER
			s.currentTerm = req.Term
		}
		// BUG: commenting out this condition allows nodes to vote for multiple candidates in a single term
		...
		if vote {
			...
			log.Println("DEBUG2:", len(s.logs), ownLastTerm)
			data, _ := json.Marshal(response{Term: s.currentTerm})
			w.Write(data)
			return
		}
