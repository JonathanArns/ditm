...
% dass das Log-Matching in allen Situationen akzeptabel funktioniert. Für das Request Matching ist die Erwartungshaltung, dass
...
beschriebenen Versuchs dar, jeweils für die vier verschiedenen Request-Matcher, die hier evaluiert werden. Für jedes Experiment
wird wowohl der Durchschnitt, als auch der Median über 10 individuelle Durchführungen des jeweiligen Versuchs angegeben.

Die vom beschriebenen Bug im SUT
...
der Requests von den vorgesehenen 10 und 100 abweicht.
% Interessant in diesen Experimenten ist zu beobachten, dass selbst bei
% ungemischten Request-Flüssen Fehler passieren, der Blick auf dem Median zeigt aber, dass dies nur in weniger als der Hälfte der
...
\begin{table}[H]
	\centering
	\caption{
        Matching von einfachen GET-Requests:
...
\begin{table}[H]
	...
	\begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|r|}
		\hline
		\multirow{2}{*}{Shuffle} & \multicolumn{2}{|c|}{num requests} & \multicolumn{2}{|c|}{heuristic} & \multicolumn{2}{|c|}{exakt} & \multicolumn{2}{|c|}{mix} & \multicolumn{2}{|c|}{counting}                                    \\ \cline{2-11}
		                         & avg                                & med                             & avg                         & med                       & avg                            & med  & avg  & med  & avg  & med  \\ \hline
...
mit Schwierigkeiten umgehen kann, sofern diese nicht zu gehäuft auftreten. Leicht identifizierbare Requests in stark gemischter
Reihenfolge sind kein Problem, genauso wie schwierig identifizierbare Requests, die in größtenteils korrekter Reihenfolge
...
und Requests an seine Grenzen kommen kann und dann effektiv unbrauchbar wird. Ganz im Gegenteil dazu stehen die Ergebnisse der
...
da die Zeitstempel, welche zur Sortierung der Tabelle von Requests und Logs verwendet werden, in diesem Teil des Systems generiert werden.
...
Daten aus einer Response in den folgenden Requests mitgesendet werden müssen. Ditm hingegen sendet im Replay den exakt gleichen
